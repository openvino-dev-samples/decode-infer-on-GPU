...
bool ParseArgsAndValidate(int argc, char *argv[], Params *params, ParamGroup group) {
    ...
    *params      = {};
    ...
    for (idx = 1; idx < argc;) {
        ...
        else if (IS_ARG_EQ(s, "bs")) {
            params->inBatchsize = int(*ValidateFileName(argv[idx++]));
            if (!params->inBatchsize) {
                return false;
            }
        }
        else if (IS_ARG_EQ(s, "nr")) {
            params->inRequest = int(*ValidateFileName(argv[idx++]));
            if (!params->inRequest) {
                return false;
            }
        }
